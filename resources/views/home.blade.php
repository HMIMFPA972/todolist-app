	...
	<div class="bg-white-300 rounded shadow p-6 m-4 w-full lg:w-3/4 lg:max-w-lg">
        <div class="mb-4">
            ...
            <form action="{{ route('add')}}" method="post">
                @csrf
                    <div class="flex mt-4">
                    <input name="title" class="shadow appearance-none border rounded w-full py-2 px-3 mr-4 text-white-darker" placeholder="Ajouter votre tâche">
        ...
        <div>
            @dd($tasks)
            @forelse ($tasks as $itemTodo )
                <div class="flex mb-4 items-center">
                    <!--@if ($itemTodo->status == 'n')
                        <p class="w-full text-grey-darkest">{{$itemTodo->title}}</p>
                    @else
                        <p class="w-full text-grey-darkest line-through">{{$itemTodo->title}}</p>
                    @endif-->
                    <p class="w-full text-grey-darkest {{$itemTodo->status=='o'?'line-through':''}}">{{$itemTodo->title}}</p>
